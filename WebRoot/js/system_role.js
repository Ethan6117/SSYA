					...
					if(l.superCode == v.code){
						if(l.name=='专题列表'){
							...
							.append($("<td class='child-module-power'></td>")
								...
								.append($("<label></label>").append(findScore(l)))
							)
							.appendTo(tbody);
						}else{
							$("<tr class='child-module'></tr>")
...
function findScore(l){
	return "<input type='checkbox' "+$.findChecked(l.score)+"  onclick='setRoleSecureValid(this, \""+l.code+"\", "+type.SCORE+")' />评分";
}
/*
 * 为角色设置模块
 * . [K]
