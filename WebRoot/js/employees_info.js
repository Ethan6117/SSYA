...
(function($){
	...
	commpny += "</tr>";
	var edu = "<tr class='edu-tr'>";
	edu += "<td><input type='text' maxlengt='64' class='width180 text ed-school' placeholder='毕业学校' /></td>";
	edu += "<td><input type='text' class='width180 text date-before ed-state-date' placeholder='入学时间' /></td>";
	edu += "<td><input type='text' class='width180 text date-before ed-end-date' placeholder='毕业时间' /></td>";
	edu += "<td><input type='text' maxlengt='64' class='width180 text ed-cation' placeholder='学历' /></td>";
	edu += "<td><input type='text' maxlengt='512' class='width180 text ed-major' placeholder='专业' /></td>";
	edu += "<td><button type='button' class='btn btn-default btn-xs' onclick='$.delEdu(this)'>移出</button></td>";
	edu += "</tr>";
	...
	// 添加教育经历
	$.addEdu = function(){
		$('tbody.edu-list').append(edu);
	};
	// 删除教育经历
	$.delEdu = function(obj){
		if($('tr.edu-tr').length <= 1) return; // 如果当前只剩下一条工作单位, 则不删除
		 $(obj).parent().parent('tr.edu-tr').remove();
	};
...
(function($){
	...
	$.analyzeEdu = function(edus){
		var edubox = $('tbody.edu-list').empty();
		$.each(edus, function(i,v){
			
			$("<tr class='edu-tr'></tr>")
			.append($("<td></td>").append("<input type='text' maxlength='64' class='width180 text ed-school' placeholder='毕业学校' value='"+v.eduSchool+"' />"))
			.append($("<td></td>").append("<input type='text' class='width180 text date-before ed-state-date' placeholder='入学时间' value='"+v.eduStartTime+"' />"))
			.append($("<td></td>").append("<input type='text' class='width180 text date-before ed-end-date' placeholder='毕业时间' value='"+v.eduLeaveTime+"' />"))
			.append($("<td></td>").append("<input type='text' maxlength='64' class='width180 text ed-cation' placeholder='学历' value='"+v.eduSchooling+"' />"))
			.append($("<td></td>").append("<input type='text' maxlength='512' class='width180 text ed-major' placeholder='专业' value='"+v.eduMajor+"' />"))
			.append($("<td></td>").append("<button type='button' class='btn btn-default btn-xs' onclick='$.delEdu(this)'>移出</button>"))
			.appendTo(edubox);
		});
	};
})(jQuery);
...
function detectionForm(){
	...
	// 详细信息
	empl.phone = $.input('.phone', true, null);	// 手机号码
	...
	empl.paySocialSecurity = $.radio('.isSocialSecurity', true, null);	// 是否缴纳社保
	
	
	empl.note = $.input('.note', false, null);	// 备注
	empl.notetype = $.radio('.clsnotetype',  true, null);
 ...
 */
function findEdu(){
	var eduLength = $('tr.edu-tr').length;
	if(eduLength < 1) return null;
	var eduArray = new Array();	// 用来封装教育经历的数组
	for(var i = 0; i < eduLength; i++){
		var edu = {};	// 工教育经历
		edu.eduSchool = $.removeTrim($('input.ed-school').eq(i).val());
		edu.eduStartTime = $.removeTrim($('input.ed-state-date').eq(i).val());
		edu.eduLeaveTime = $.removeTrim($('input.ed-end-date').eq(i).val());
		edu.eduSchooling = $.removeTrim($('input.ed-cation').eq(i).val());
		edu.eduMajor = $.removeTrim($('input.ed-major').eq(i).val());
		if(edu.eduSchool || edu.eduStartTime || edu.eduLeaveTime || edu.eduSchooling || edu.eduMajor) eduArray.push(edu);
	}
	return eduArray;
}
/*
...
$.findEmployeesInfo = function(emplId){
	...
	$.getJSON('./mgr/employees/internship/findEmployeesInfo',{emplId:emplId}, function(data){
		...
		$('input.isSocialSecurity').eq(info.isSocialSecurity ? 1 : 0).prop('checked', true);
		$('input.note').val(info.note);
		$('input.clsnotetype').each(function(){
			
			if($(this).val()==info.notetype){
		...
		$.analyzeEdu(data.body.edus); // 教育经历 
		...
		alert("data:"+ data.body.departments);*/
		...
		if(info.state == 3 || info.state == 4){
			...
			var bdg=BootstrapDialog.show({
				...
				closable : true,
				message : '该员工已 <b>离职</b> 或 <b>淘汰</b>, 所有信息不能编辑!'
			});
			setTimeout(function(){
				bdg.close();
			},1500);
