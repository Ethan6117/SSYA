...
public class ModuleServiceImpl implements ModuleService {
	...
	public JSONReturn findMenu(String acctName) {
		...
		/*for (TeModule teModule : moduleList) {
			if (teModule.getModuleName().equals("个人资料")) {
				teModule.setModulePage("mgr/modifyInfo?acctName="+acctName);
			}
		}*/
	...
	public JSONReturn findModuleParameter(String moduleCode, String acctName) {
		...
		if ("0".equals(moduleCode)) {
			...
			map.put("score", teAccount.getAcctSuper() ? true : false);
		...
		// 获取当前模块名与上级模块名, 前台做为显示
		map = findModuleName(map, moduleCode, teModule);
		boolean add = false, del = false, modify = false, find = false, score = false;
		...
		map.put("score", teAccount.getAcctSuper() ? true : score);
		return JSONReturn.buildSuccess(map);
	}
	...
	public JSONReturn setRoleSecureValid(long rold, String code, int type, boolean add) {
		...
		if (CompareUtil.isEmpty(rm)) {
			rm = new TeRoleModule(role.getRoleLabel(), code, mo.getModuleSuperCode(), false, false, false, false ,false);
